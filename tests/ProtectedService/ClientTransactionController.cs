...
public class TransactionSummary
{
  ...
  public TransactionSummary(string transactionId, string verificationId)
  {
  ...
  /*
   * This represents the Customer-facing transaction being executed.
   */
  public string? TransactionId;
  
  /*
   *  Verification ID that should really be cached on the server side
   * based upon the transactionId.  For simplicity we pass it from the
   * front end, but in a real application it should be stored in the
   * server context
   */
      ...
      )
    {
      try
      {
        ...
        var dbResponse = await dodgeball.Checkpoint(
          ...
          /*
           *  Indicates whether this is a fresh invocation, or a
           *  resumption after letting the Front End control MFA or
           *  similar operations.
           */
          priorVerificationId
        );
